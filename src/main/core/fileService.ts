...
export async function getStoragePoints(deviceId: string) {
  ...
  try {
    ...
    const internalLine = lines.find(l => l.includes(' /data')) || lines.find(l => l.includes('/storage/emulated')) || lines.find(l => l.includes('/sdcard'))
    ...
    if (storagePoints.length === 0) {
      storagePoints.push({ name: 'Bộ nhớ trong', path: '/sdcard', type: 'internal', total: 0, used: 0, percent: 0 })
    }
