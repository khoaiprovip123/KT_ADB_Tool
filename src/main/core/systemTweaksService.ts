...
export async function getBloatwareWithStatus(deviceId: string): Promise<BloatwareWithStatus[]> {
...
export async function getTweakStatus(deviceId: string, tweak: SystemTweak): Promise<boolean> {
...
export async function getCurrentResolution(deviceId: string): Promise<{ width: number; height: number } | null> {
  try {
...
export async function setDpi(deviceId: string, dpi: number): Promise<{ success: boolean; message: string }> {
  if (dpi < 100 || dpi > 640) {
...
export async function resetDpi(deviceId: string): Promise<{ success: boolean; message: string }> {
  try {
...
export async function resetResolution(deviceId: string): Promise<{ success: boolean; message: string }> {
  try {
...
export async function setAnimationScale(deviceId: string, scale: 0 | 0.5 | 1.0): Promise<void> {
  const keys = ['window_animation_scale', 'transition_animation_scale', 'animator_duration_scale']
  await Promise.all(keys.map((k) => execAdb(deviceId, `shell settings put global ${k} ${scale}`)))
}

// ─── BUILT-IN BLOATWARE DB (Fallback) ────────────────────────────────────────

const BUILTIN_BLOATWARE_DB: BloatwareEntry[] = [
  { package: 'com.miui.analytics', name: 'MIUI Analytics', description: 'Thu thập dữ liệu sử dụng gửi về Xiaomi', risk: 'SAFE', category: 'Telemetry' },
  { package: 'com.miui.msa.global', name: 'MSA (MIUI System Ads)', description: 'Dịch vụ quảng cáo hệ thống MIUI', risk: 'SAFE', category: 'Ads' },
  { package: 'com.xiaomi.joyose', name: 'Joyose', description: 'Thermal controller', risk: 'RISKY', category: 'System Service', preferDisable: true },
  { package: 'com.miui.daemon', name: 'MIUI Daemon', description: 'System Daemon phân tích dữ liệu nền', risk: 'SAFE', category: 'Telemetry', preferDisable: true },
  { package: 'com.miui.hybrid', name: 'Quick App Service', description: 'Framework Quick App — nguồn gốc quảng cáo trong app system', risk: 'SAFE', category: 'Ads', preferDisable: true },
  { package: 'com.mi.globalminusscreen', name: 'App Vault / Minus Screen', description: 'Màn hình gạt trái trên Launcher', risk: 'SAFE', category: 'Bloatware' },
  { package: 'com.miui.videoplayer', name: 'Mi Video', description: 'Trình phát video Xiaomi có quảng cáo', risk: 'SAFE', category: 'Media' },
  { package: 'com.miui.player', name: 'Mi Music', description: 'Ứng dụng nghe nhạc Xiaomi có quảng cáo', risk: 'SAFE', category: 'Media' },
  { package: 'com.miui.yellowpage', name: 'Xiaomi Yellow Pages', description: 'Danh bạ vàng Xiaomi — ít dùng', risk: 'SAFE', category: 'Bloatware' },
  { package: 'com.miui.miservice', name: 'Mi Service Framework', description: 'Framework dịch vụ Xiaomi', risk: 'RISKY', category: 'System Service', preferDisable: true },
  { package: 'com.xiaomi.payment', name: 'Mi Payment Center', description: 'Trung tâm thanh toán Xiaomi', risk: 'SAFE', category: 'Finance' },
  { package: 'com.xiaomi.aicr', name: 'Xiaomi AI Engine', description: 'AI engine nền của Xiaomi', risk: 'RISKY', category: 'AI Service', preferDisable: true },
  { package: 'com.miui.cloudservice', name: 'MIUI Cloud Service', description: 'Đồng bộ dữ liệu lên Xiaomi Cloud', risk: 'SAFE', category: 'Cloud' },
  { package: 'com.miui.cloudbackup', name: 'MIUI Cloud Backup', description: 'Sao lưu lên Xiaomi Cloud', risk: 'SAFE', category: 'Cloud' },
  { package: 'com.miui.compass', name: 'Mi Compass', description: 'La bàn Xiaomi', risk: 'SAFE', category: 'Preinstalled App' },
  { package: 'com.miui.calculator', name: 'Mi Calculator', description: 'Máy tính Xiaomi', risk: 'SAFE', category: 'Preinstalled App' },
  { package: 'cn.wps.xiaomi.abroad.lite', name: 'Xiaomi WPS Reader', description: 'Ứng dụng đọc tài liệu WPS Lite', risk: 'SAFE', category: 'Preinstalled App' },
  { package: 'com.miui.bugreport', name: 'Bug Report Tool', description: 'Công cụ báo lỗi Xiaomi', risk: 'SAFE', category: 'System Tool' },
  { package: 'com.android.egg', name: 'Android Easter Egg', description: 'Easter Egg ẩn của Android', risk: 'SAFE', category: 'Bloatware' },
  { package: 'com.android.traceur', name: 'Android Traceur', description: 'Công cụ trace hệ thống', risk: 'SAFE', category: 'System Tool' },
  { package: 'com.android.stk', name: 'SIM Toolkit', description: 'SIM Toolkit', risk: 'RISKY', category: 'Carrier' },
  { package: 'com.android.carrierdefaultapp', name: 'Carrier Default App', description: 'App mặc định nhà mạng', risk: 'RISKY', category: 'Carrier' },
  { package: 'com.android.managedprovisioning', name: 'Managed Provisioning', description: 'Dự phòng quản lý doanh nghiệp', risk: 'SAFE', category: 'Enterprise' },
  { package: 'com.google.android.as.oss', name: 'Google Private Compute Services', description: 'Service AI của Google chạy nền', risk: 'RISKY', category: 'Google', preferDisable: true }
]
