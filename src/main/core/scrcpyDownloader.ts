...
const DL_URL =
  "https://github.com/Genymobile/scrcpy/releases/download/v2.4/scrcpy-win64-v2.4.zip";

export async function ensureScrcpy(
  binPath: string,
  ...
  try {
    ...
    if (stats.size < 1024 * 1024) {
      throw new Error(
        `Tệp tin tải về không hoàn chỉnh (Kích thước: ${(stats.size / 1024).toFixed(1)} KB). Vui lòng thử lại.`,
      );
    }
    ...
    if (fs.existsSync(extractedNested)) {
      ...
      try {
        fs.rmSync(extractedNested, { recursive: true, force: true });
      } catch (e) {
        /* ignore */
      }
    }
