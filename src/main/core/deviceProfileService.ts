 ...
 */
export async function getInstalledPackageSet(deviceId: string): Promise<Set<string>> {
  try {
...
export async function readDeviceConfigSnapshot(
  ...
  try {
    ...
    console.warn('device_config list not supported or failed, returning empty:', error)
    return {}
  }
}
