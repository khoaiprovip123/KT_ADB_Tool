...
export async function manageApp(
  ...
  try {
    ...
    let cmd = ''
    switch (action) {
      case 'uninstall': cmd = `pm uninstall -k --user 0 ${pkgName}`; break
      case 'disable':   cmd = `pm disable-user --user 0 ${pkgName}`; break
      case 'enable':    cmd = `pm enable ${pkgName}`; break
      case 'restore':   cmd = `cmd package install-existing ${pkgName}`; break
      case 'clear':     cmd = `pm clear ${pkgName}`; break
      case 'stop':      cmd = `am force-stop ${pkgName}`; break
    }
