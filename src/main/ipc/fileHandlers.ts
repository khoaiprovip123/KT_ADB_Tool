...
export function registerFileHandlers(mainWindow: Electron.BrowserWindow) {
  ipcMain.handle('adb:list-directory', async (_event, { deviceId, remotePath }) => {
    return await listDirectory(deviceId, remotePath)
  })

  ipcMain.handle('adb:create-directory', async (_event, { deviceId, remotePath }) => {
    return await createDirectory(deviceId, remotePath)
  })

  ipcMain.handle('adb:delete-file', async (_event, { deviceId, remotePath }) => {
    return await deleteFile(deviceId, remotePath)
  })

  ipcMain.handle('adb:rename-file', async (_event, { deviceId, oldPath, newPath }) => {
    return await renameFile(deviceId, oldPath, newPath)
  })

  ipcMain.handle('adb:push-file', async (_event, { deviceId, localPath, remotePath }) => {
  ...
  ipcMain.handle('adb:pull-file', async (_event, { deviceId, remotePath, localPath }) => {
  ...
  ipcMain.handle('adb:get-file-base64', async (_event, { deviceId, remotePath }) => {
