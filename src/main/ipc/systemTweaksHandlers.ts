...
export function registerSystemTweaksHandlers(mainWindow: Electron.BrowserWindow) {
  // ── Bloatware DB ───────────────────────────────────────────────────────────
  ...
  ipcMain.handle('adb:debloat-package', async (_event, deviceId: string, packageName: string, action: string, preferDisable: boolean) => {
    return debloatPackage(deviceId, packageName, action as DebloatAction, preferDisable)
  })

  ipcMain.handle('adb:batch-debloat', async (_event, deviceId: string, packages: Array<{ package: string; preferDisable?: boolean }>, action: string) => {
    return batchDebloat(
      deviceId,
      packages,
