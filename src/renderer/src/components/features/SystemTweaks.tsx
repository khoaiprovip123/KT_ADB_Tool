...
export function SystemTweaks() {
  ...
  const [activeCategory, setActiveCategory] =
    useState<TweakCategory>("debloat");

  // Debloat States
  const [bloatListWithStatus, setBloatListWithStatus] = useState<
    BloatwareEntry[]
  >([]);
  ...
  const [verifyAdbInstallsEnabled, setVerifyAdbInstallsEnabled] =
    useState(false);
  ...
  const handleBatchDebloatAction = async (
    ...
    try {
      ...
      const successCount = results.filter(
        (r: { package: string; success: boolean; message: string }) =>
          r.success,
      ).length;
    ...
    return (
      <div className="absolute inset-4 lg:inset-6 flex flex-col items-center justify-center p-8 bg-[#f8fafc]/90 backdrop-blur-3xl rounded-[32px] border border-slate-200/80 shadow-[0_20px_50px_rgba(0,0,0,0.06)] text-slate-800">
        ...
        <p className="text-slate-500 text-center max-w-sm text-sm font-semibold leading-relaxed">
          Vui lòng kết nối thiết bị Android bằng cáp hoặc Wi-Fi và cấp quyền gỡ
          lỗi USB để tiếp tục tinh chỉnh hệ thống.
        </p>
      </div>
    );
  }
  ...
  const riskyCount = bloatListWithStatus.filter(
    (p) => p.risk === "RISKY",
  ).length;
  ...
  const allSelected =
    selectedBloat.size === selectableCount && selectableCount > 0;
    ...
    <div className="absolute inset-4 lg:inset-6 flex flex-col overflow-hidden bg-[#f8fafc]/90 backdrop-blur-3xl rounded-[32px] p-5 lg:p-6 border border-slate-200/80 shadow-[0_20px_50px_rgba(0,0,0,0.06)] text-slate-800">
      ...
      <div className="bg-white/80 border border-slate-200/80 p-4 rounded-[24px] shadow-sm flex items-center justify-between gap-4 shrink-0 mb-4 select-none">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-[9px] font-black text-blue-650 uppercase tracking-widest block">
              SYSTEM TWEAKS
            </span>
                  ...
                  {loading && bloatListWithStatus.length === 0 ? (
                    <div className="p-6 space-y-3">
                      {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div
                          key={i}
                          className="h-14 bg-slate-100/50 rounded-2xl animate-pulse"
                        />
                      ))}
                    </div>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8 min-h-[300px]">
                      ...
                      <p className="text-sm font-black text-slate-700">
                        Tuyệt vời! Thiết bị cực kỳ sạch sẽ.
                      </p>
                      <p className="text-xs font-semibold text-slate-400 mt-1">
                        Không tìm thấy ứng dụng rác nào phù hợp với bộ lọc hiện
                        tại.
                      </p>
                    </div>
                  ) : (
                        ...
                        >
                          <div className="col-span-5 flex items-center gap-3 truncate">
                            <button
                              disabled={isKeep}
                              onClick={() =>
                                toggleSelect(item.package, item.risk)
                              }
                          ...
                          <div className="col-span-2 flex justify-center">
                            <span
                              className={`px-2.5 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-wider ${classificationBadge}`}
                            >
                              {item.risk}
                            </span>
                          </div>
                          ...
                          <div className="col-span-3 flex items-center justify-end gap-2">
                            <span
                              className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-wider ${statusBadge}`}
                            >
                              {statusText}
                            </span>
                            ...
                            <div className="hidden group-hover:flex items-center gap-1">
                              {item.status !== "uninstalled" ? (
                                ...
                                <ActionButton
                                  ...
                                  loading={
                                    actionLoading === `restore-${item.package}`
                                  }
                                />
                              )}
                            </div>
                      ...
                      <div className="flex items-center gap-2">
                        {batchProgress ? (
                          <div className="flex items-center gap-3 bg-blue-50 border border-blue-150 px-4 py-2 rounded-xl">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-blue-650">
                              Xử lý {batchProgress.current}/
                              {batchProgress.total}
                            </span>
                          </div>
                        ) : (
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() =>
                                handleBatchDebloatAction("uninstall")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleBatchDebloatAction("disable")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleBatchDebloatAction("restore")
                              }
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center text-xs font-bold text-slate-500">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-blue-655 font-black">
                              {customDpi} DPI
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <PresetButton
                            active={customDpi === 360}
                            onClick={() => setCustomDpi(360)}
                            label="Nhỏ (360)"
                          />
                          <PresetButton
                            active={customDpi === 440}
                            onClick={() => setCustomDpi(440)}
                            label="Mặc định"
                          />
                          <PresetButton
                            active={customDpi === 500}
                            onClick={() => setCustomDpi(500)}
                            label="Lớn (500)"
                          />
                        </div>

                        <p className="text-[10px] text-slate-400 font-semibold leading-relaxed">
                          ⚠️ Lưu ý: Việc đặt DPI quá nhỏ hoặc quá lớn có thể làm
                          méo biểu tượng hoặc đè lớp UI hiển thị.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="bg-white/90 p-6 rounded-3xl border border-slate-200/80 shadow-sm relative overflow-hidden flex flex-col justify-between">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <p className="text-[10px] text-slate-400 font-semibold leading-relaxed">
                          ⚠️ Lưu ý: Thay đổi độ phân giải sai tỷ lệ có thể khiến
                          màn hình bị giãn hoặc mất cảm ứng tạm thời.
                        </p>
                      </div>
                    </div>
                ...
                <div className="bg-white/90 p-6 rounded-3xl border border-slate-200/80 shadow-sm relative overflow-hidden space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-450 font-semibold">
                      Tăng độ bảo mật dữ liệu cá nhân bằng cách khóa các tiến
                      trình thu thập ẩn.
                    </p>
                  </div>
              ...
              >
                <div className="bg-white/90 p-6 rounded-3xl border border-slate-200/80 shadow-sm relative overflow-hidden space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-slate-450 font-semibold">
                      Tăng hoặc giảm thời gian chờ của animation. Animation càng
                      nhanh, cảm giác máy phản hồi càng tốc độ.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                    <AnimScaleCard
                      ...
                      loading={
                        actionLoading === "apply-anim" && animScale === 0
                      }
                    />
                    <AnimScaleCard
                      label="Siêu mượt"
                      ...
                      loading={
                        actionLoading === "apply-anim" && animScale === 0.5
                      }
                    />
                    <AnimScaleCard
                      label="Mặc định"
                      ...
                      loading={
                        actionLoading === "apply-anim" && animScale === 1.0
                      }
                    />
                    <AnimScaleCard
                      label="Chậm rãi"
                      ...
                      loading={
                        actionLoading === "apply-anim" && animScale === 1.5
                      }
                    />
                  </div>
                </div>
              ...
              >
                <div className="bg-white/90 p-6 rounded-3xl border border-slate-200/80 shadow-sm relative overflow-hidden space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-slate-450 font-semibold">
                      Tinh chỉnh độ nhạy điểm chạm và phản hồi con trỏ trên hệ
                      điều hành.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-semibold leading-relaxed">
                          Tùy chỉnh tốc độ di chuyển con trỏ (khi dùng chuột/bàn
                          rê Bluetooth).
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        <input
                          ...
                          value={pointerSpeed}
                          onChange={(e) =>
                            setPointerSpeed(Number(e.target.value))
                          }
                          className="flex-1 h-2 bg-slate-100 rounded-full appearance-none cursor-pointer accent-indigo-650"
                        />
                        <span className="w-12 text-center text-xs font-black text-indigo-650 bg-indigo-50 border border-indigo-100 px-2 py-1 rounded-lg">
                          {pointerSpeed}
                        </span>
                      </div>
                      ...
                      >
                        {actionLoading === "pointer_speed"
                          ? "Đang cập nhật..."
                          : "Cập nhật tốc độ con trỏ"}
                      </button>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="h-12 bg-slate-50 border border-slate-150 rounded-xl flex items-center justify-center text-[10px] font-bold text-slate-500">
                        Giá trị con trỏ hợp lệ: Từ -7 (Chậm nhất) đến +7 (Nhanh
                        nhất).
                      </div>
                    </div>
                  </div>
              ...
              >
                ...
                <div className="bg-white/90 p-6 rounded-3xl border border-slate-200/80 shadow-sm relative overflow-hidden space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-slate-450 font-semibold">
                      Điều khiển cách hệ thống Android giữ tiến trình ngầm và
                      giải phóng RAM.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-semibold leading-relaxed">
                          Giảm tiến trình chạy nền sẽ giải phóng nhiều RAM hơn
                          cho app hiện tại.
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <PresetButton
                          active={bgLimit === -1}
                          onClick={() => applyBgLimit(-1)}
                          label="Tiêu chuẩn"
                        />
                        <PresetButton
                          active={bgLimit === 0}
                          onClick={() => applyBgLimit(0)}
                          label="Không có"
                        />
                        <PresetButton
                          active={bgLimit === 1}
                        ...
                        <PresetButton
                          active={bgLimit === 2}
                          onClick={() => applyBgLimit(2)}
                          label="2 tiến trình"
                        />
                        <PresetButton
                          active={bgLimit === 3}
                          onClick={() => applyBgLimit(3)}
                          label="3 tiến trình"
                        />
                        <PresetButton
                          active={bgLimit === 4}
                          onClick={() => applyBgLimit(4)}
                          label="4 tiến trình"
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="p-4 bg-slate-50 border border-slate-150 rounded-2xl text-[10px] text-slate-550 leading-relaxed font-semibold">
                        💡 **Lời khuyên**: Đối với các dòng máy yếu (RAM 3GB -
                        4GB), hãy đặt giới hạn ở mức **3 hoặc 4 tiến trình** để
                        cải thiện đáng kể độ mượt của game mà không gây trễ tin
                        nhắn Zalo/Messenger quá nặng.
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-white/90 p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-slate-450 font-semibold">
                      Đưa ứng dụng vào danh sách loại trừ tối ưu pin (Doze
                      Whitelist) để nhận thông báo tức thời.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        disabled={
                          !pkgNotifyInput || actionLoading === "fix_notify"
                        }
                      ...
                      >
                        {actionLoading === "fix_notify"
                          ? "Đang sửa..."
                          : "Sửa trễ thông báo"}
                      </button>
                    </div>
                    <p className="text-[10px] text-slate-400 font-semibold leading-relaxed">
                      💡 **Mẹo**: Lệnh này sẽ tự động loại bỏ ứng dụng khỏi chế
                      độ ngủ tiết kiệm pin (Doze mode), cho phép chạy nền và
                      nâng bucket ưu tiên lên hoạt động tích cực (standby-bucket
                      active).
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-white/90 p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-slate-450 font-semibold">
                      Khóa quyền chạy ngầm của các ứng dụng không cần thiết nhằm
                      tiết kiệm năng lượng triệt để.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-3">
                      ...
                      <div className="flex gap-2 shrink-0">
                        <button
                          ...
                          disabled={
                            !pkgFreezeInput || actionLoading === "freeze_bg"
                          }
                        ...
                        <button
                          ...
                          disabled={
                            !pkgFreezeInput || actionLoading === "unfreeze_bg"
                          }
                    ...
                    <p className="text-[10px] text-slate-400 font-semibold leading-relaxed">
                      ⚠️ **Lưu ý**: Đóng băng nền sẽ chặn quyền
                      `RUN_IN_BACKGROUND` của gói thông qua AppOps và chuyển
                      trạng thái standby thành `restricted`. Ứng dụng sẽ không
                      thể gửi tin nhắn hoặc đẩy dịch vụ nền trừ khi được bạn mở
                      trực tiếp.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50/50 p-6 rounded-3xl border border-blue-100 shadow-sm space-y-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-500 leading-relaxed font-semibold">
                        Do cơ chế khóa ứng dụng đa nhiệm (Recent lock) nằm sâu
                        trong ứng dụng bảo mật độc quyền của Xiaomi, **không có
                        lệnh ADB công khai** nào có thể trực tiếp thực hiện tính
                        năng này. Bạn có thể thiết lập bằng cách thủ công sau:
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="pl-14 space-y-2 text-xs text-slate-600 font-semibold list-decimal leading-relaxed">
                    <p>
                      🔹 **Cách 1 (Từ ứng dụng Bảo mật - Khuyên dùng cho
                      HyperOS):**
                    </p>
                    <p className="pl-4">
                      1. Mở ứng dụng **Bảo mật (Security)** mặc định trên điện
                      thoại.
                    </p>
                    <p className="pl-4">
                      2. Nhấn vào biểu tượng **Cài đặt (Bánh răng)** ở góc trên
                      bên phải.
                    </p>
                    <p className="pl-4">
                      3. Chọn mục **Tăng tốc (Boost speed)** → **Khóa ứng dụng
                      (Lock apps)**.
                    </p>
                    <p className="pl-4">
                      4. Gạt bật công tắc cho các ứng dụng bạn muốn khóa lại.
                    </p>

                    <p className="pt-2">
                      🔹 **Cách 2 (Từ màn hình Đa nhiệm - Dành cho MIUI cũ):**
                    </p>
                    <p className="pl-4">
                      1. Vuốt lên giữ để mở màn hình Đa nhiệm gần đây.
                    </p>
                    <p className="pl-4">
                      2. Ấn giữ lâu vào thẻ xem trước của ứng dụng mong muốn.
                    </p>
                    <p className="pl-4">
                      3. Nhấn vào **biểu tượng Ổ khóa** để giữ ứng dụng chạy
                      ngầm vĩnh viễn.
                    </p>
                  </div>
                </div>
...
function CategoryItem({
  ...
  return (
    ...
    >
      <div
        className={`p-2 rounded-xl shrink-0 ${active ? "bg-white/10 text-white" : "bg-slate-100 text-slate-500"}`}
      >
        {icon}
      </div>
...
function StatCard({
  ...
  return (
    <div
      className={`px-2.5 py-1 rounded-xl border flex items-center gap-1.5 text-[9px] font-black uppercase tracking-wider ${color}`}
    >
      <span>{label}:</span>
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        <span
          className={`text-[10px] font-black uppercase tracking-widest ${active ? "text-purple-200" : "text-slate-400"}`}
        >
          {label}
        </span>
        {loading ? (
          ...
          <span
            className={`text-base font-black ${active ? "text-white" : "text-purple-600"}`}
          >
            {scale}x
          </span>
        )}
      ...
      <p
        className={`text-[10px] font-bold ${active ? "text-purple-100" : "text-slate-500"}`}
      >
        {desc}
      </p>
    </button>
  ...
  return (
    <div className="flex items-center justify-between py-6 gap-6 transition-all hover:bg-slate-50/20 px-4 -mx-4 rounded-xl">
      ...
      >
        ...
        >
          {loading && (
            <RefreshCcw size={10} className="animate-spin text-blue-600" />
          )}
        </motion.div>
      </button>
    </div>
