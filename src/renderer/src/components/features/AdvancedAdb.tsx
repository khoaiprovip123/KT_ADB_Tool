...
export function AdvancedAdb() {
  ...
  const [catalogProcessingId, setCatalogProcessingId] = useState<string | null>(
    null,
  );
  const [catalogOutput, setCatalogOutput] = useState<string>("");
  const [isCatalogTerminalMaximized, setIsCatalogTerminalMaximized] =
    useState(false);
  ...
  const [isShellTerminalMaximized, setIsShellTerminalMaximized] =
    useState(false);
    ...
    const commandList: AdvancedCommandDefinition[] = [
      {
        ...
        description:
          "Chẩn đoán sức khỏe pin, dung lượng hiện tại, nhiệt độ và dòng sạc qua dumpsys battery.",
      ...
      },
      {
        ...
        description:
          "Xem trạng thái WakeLocks, độ sáng màn hình hiện tại và các yếu tố tiêu thụ điện năng.",
        category: "diagnostics",
        risk: "SAFE",
        mode: "READ_ONLY",
      ...
      },
      {
        ...
        description:
          "Xác định nhanh ứng dụng và activity đang chạy trên màn hình hiện tại.",
        category: "diagnostics",
        risk: "SAFE",
        mode: "READ_ONLY",
        readTemplate:
      ...
      },
      {
        ...
        description:
          "Cấp quyền can thiệp settings hệ thống bảo mật cao cho ứng dụng (thường dùng cho SetEdit, Tasker, v.v.).",
        ...
        needsConfirmText:
          "Quyền WRITE_SECURE_SETTINGS cho phép ứng dụng can thiệp trực tiếp vào cài đặt hệ thống sâu và bảo mật. Chỉ cấp cho ứng dụng đáng tin cậy!",
      },
      {
        id: "perm_grant_runtime",
        title: "Cấp quyền Runtime nâng cao",
        description:
          "Ép buộc cấp quyền chạy (như đọc ảnh, định vị, máy ảnh...) mà không cần hỏi trên màn hình.",
        category: "permissions",
        risk: "MEDIUM",
        mode: "PACKAGE_OP",
      ...
      },
      {
        ...
        description:
          'Đổi máy chủ kiểm tra kết nối mạng sang Apple/Google để loại bỏ dấu chấm than hoặc thông báo "Không có Internet" phiền toái bên cạnh biểu tượng WiFi tại Việt Nam.',
        category: "network",
        risk: "SAFE",
        mode: "WRITE_SETTING",
      ...
      },
      {
        ...
        description:
          "Kiểm soát khả năng hoạt động ngầm của ứng dụng để tiết kiệm pin.",
        category: "appops",
        risk: "MEDIUM",
        mode: "PACKAGE_OP",
        ...
        needsConfirmText:
          "Thao tác chặn chạy ngầm có thể khiến ứng dụng không nhận được thông báo kịp thời.",
      },
      {
        id: "appops_wake_lock",
        title: "Chặn WakeLock (WAKE_LOCK)",
        description:
          "Chặn ứng dụng tự động đánh thức màn hình hoặc giữ thiết bị hoạt động ở chế độ chờ.",
        category: "appops",
        risk: "RISKY",
        mode: "PACKAGE_OP",
        ...
        needsConfirmText:
          "Chặn WakeLock có thể gây ngắt quãng hoặc lỗi chạy ngầm của các ứng dụng nghe nhạc/GPS.",
      },
      {
        id: "appops_draw_overlay",
        title: "Quyền hiển thị trên ứng dụng khác",
        description:
          "Bật/Tắt trực tiếp quyền hiển thị cửa sổ nổi (SYSTEM_ALERT_WINDOW) của ứng dụng.",
        category: "appops",
        risk: "RISKY",
        mode: "PACKAGE_OP",
      ...
      },
      {
        ...
        description:
          "Cấu hình tối đa số lượng tiến trình được chạy ngầm trong RAM (mặc định: standard).",
        category: "settings",
        risk: "RISKY",
        mode: "WRITE_SETTING",
      ...
      },
      {
        ...
        description:
          "Đóng băng và giải phóng toàn bộ RAM của ứng dụng được chọn (force-stop).",
        category: "components",
        risk: "MEDIUM",
        mode: "PACKAGE_OP",
      ...
      },
      {
        ...
        description:
          "Khôi phục ứng dụng về trạng thái mới cài đặt (Clear Data). Toàn bộ dữ liệu của ứng dụng sẽ bị xóa sạch.",
        category: "components",
        risk: "DANGEROUS",
        mode: "PACKAGE_OP",
        applyTemplate: "pm clear {package}",
        needsConfirmText:
          "Hành động này sẽ XÓA SẠCH toàn bộ dữ liệu, tài khoản đăng nhập của ứng dụng này và không thể phục hồi!",
      },
      {
        id: "net_ip_addr",
        title: "Xem địa chỉ IP mạng",
        description:
          "Hiển thị các giao diện mạng Wi-Fi/4G và địa chỉ IP cục bộ hiện tại.",
        category: "network",
        risk: "SAFE",
        mode: "READ_ONLY",
      ...
      },
      {
        ...
        description:
          "Liệt kê các socket mạng đang mở hoặc đang trao đổi dữ liệu ra internet.",
        category: "network",
        risk: "SAFE",
        mode: "READ_ONLY",
  ...
  const loadHudInfo = async () => {
    ...
    try {
      ...
      const batteryRes = await window.api.executeRawShell(
        activeDevice,
        "dumpsys battery",
      );
      ...
      const ipRes = await window.api.executeRawShell(
        activeDevice,
        "ip route get 1.1.1.1 || ip address show wlan0",
      );
      if (ipRes.success && ipRes.output) {
        const ipMatch =
          ipRes.output.match(/src\s+([0-9.]+)/) ||
          ipRes.output.match(/inet\s+([0-9.]+)/);
  ...
  const loadProfileData = async () => {
    ...
    try {
      if (activeProfileTab === "props") {
        ...
        const namespace = activeProfileTab.replace("settings_", "") as
          | "global"
          | "secure"
          | "system";
      ...
      {!activeDevice ? (
        <div className="text-center p-12 bg-white/60 backdrop-blur-2xl rounded-[32px] border border-slate-200/60 shadow-md flex-1 flex flex-col items-center justify-center min-h-[400px]">
          ...
          <p className="text-slate-500 max-w-sm text-sm font-medium">
            Kết nối thiết bị Android bằng cáp hoặc Wi-Fi và cấp quyền Gỡ lỗi USB
            để kích hoạt bảng điều khiển nâng cao.
          </p>
        </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden gap-4">
              ...
              <div>
                ...
                {deviceProfile && (
                  <p className="text-[10px] font-bold text-slate-450 mt-0.5">
                    Android {deviceProfile.release} • SDK {deviceProfile.sdk} •
                    Serial: {activeDevice}
                  </p>
                )}
              </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-2.5 bg-white border border-slate-200/60 rounded-[22px] shadow-sm select-none shrink-0">
            <div className="flex items-center gap-6 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                <Zap
                  className={`w-4 h-4 ${hudBattery.charging ? "text-amber-500 animate-bounce" : "text-slate-400"}`}
                />
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">
                  PIN:
                </span>
                <span className="text-[10px] font-mono font-extrabold text-slate-700">
                  {hudBattery.level}%{" "}
                  <span className="text-[9px] text-slate-450 font-sans font-bold">
                    ({hudBattery.temp}°C)
                  </span>
                </span>
              </div>
                    ...
                    {[
                      ...
                      >
                        {pTab === "props"
                          ...
                          : pTab.replace("settings_", "Settings ")}
                      </button>
                    ))}
                  </div>
                        ...
                        {filteredProps.map((p, idx) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2 max-w-full sm:max-w-[65%] shrink-0">
                              ...
                              <button
                                onClick={() =>
                                  handleCopyToClipboard(p.value, p.key)
                                }
                      ...
                      {filteredSettings.map((s, idx) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 max-w-full sm:max-w-[65%] shrink-0">
                            ...
                            <button
                              onClick={() =>
                                handleCopyToClipboard(s.value, s.key)
                              }
            ...
            {subTab === "catalog" && (
              <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-5 gap-5 overflow-hidden mt-4 relative">
                          ...
                          >
                            <div>
                              ...
                              <h4 className="font-extrabold text-[14px] leading-snug text-slate-800 group-hover:text-indigo-600 transition-colors flex items-center gap-1.5">
                                {cmd.id === "net_fix_captive_portal" && (
                                  <Wifi className="w-4 h-4 text-emerald-500" />
                                )}
                                {cmd.id === "perm_write_secure_settings" && (
                            ...
                            <div className="flex gap-2 justify-end border-t border-slate-100 pt-3 mt-4 shrink-0 select-none">
                              {cmd.readTemplate && (
                                <button
                                  onClick={() =>
                                    handleExecutePreset(cmd.id, "read")
                                  }
                              ...
                              {cmd.applyTemplate && (
                                <button
                                  onClick={() =>
                                    handleExecutePreset(cmd.id, "apply")
                                  }
                                  disabled={catalogProcessingId === cmd.id}
                              ...
                              {cmd.rollbackTemplate && (
                                <button
                                  onClick={() =>
                                    handleExecutePreset(cmd.id, "rollback")
                                  }
                                  disabled={catalogProcessingId === cmd.id}
                  ...
                  <div
                    className={`border border-slate-200/80 rounded-2xl flex flex-col overflow-hidden bg-slate-950 relative shadow-inner transition-all duration-300 ${isCatalogTerminalMaximized ? "col-span-5 h-full" : "col-span-1 lg:col-span-2"}`}
                  >
                      ...
                      {catalogOutput || (
                        <span className="text-slate-500 italic select-none">
                          * Chưa có dữ liệu đầu ra.
                          <br />* Chọn &quot;Đọc giá trị&quot; hoặc &quot;Áp
                          dụng&quot; từ danh sách preset để chạy và hiển thị kết
                          quả chẩn đoán tại đây.
                        </span>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                {!rawShellUnlocked ? (
                  <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-md mx-auto min-h-[350px]">
                    ...
                    <h3 className="text-lg font-black text-slate-800 tracking-tight select-none">
                      Khu Vực Kiểm Thử Cấp Cao
                    </h3>
                    <p className="text-xs text-slate-500 mt-2 leading-relaxed font-semibold">
                      Sử dụng Raw ADB Shell cho phép bạn can thiệp trực tiếp vào
                      phần cứng thiết bị. Các câu lệnh sai có thể gây lỗi hệ
                      thống nghiêm trọng, Bootloop (treo logo), hoặc mất dữ
                      liệu.
                    </p>
                    ...
                    <div className="w-full mt-6 select-none">
                      <div className="relative w-full h-12 bg-slate-50 border border-slate-200/80 rounded-2xl flex items-center justify-center overflow-hidden shadow-inner">
                        ...
                        <div
                          ...
                          style={{
                            clipPath: `inset(0 ${100 - slideValue}% 0 0)`,
                          }}
                        ></div>
                        {/* Shimmer text */}
                        ...
                        <div
                          ...
                          style={{
                            left: `calc(${slideValue}% - ${slideValue * 0.44}px)`,
                          }}
                        >
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden bg-slate-950 rounded-2xl border border-slate-900 shadow-2xl relative">
                    ...
                    <div className="bg-slate-900/90 border-b border-slate-950 px-5 py-2.5 flex justify-between items-center shrink-0 z-20 select-none">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-extrabold font-mono text-slate-300 ml-2 tracking-wider">
                          KT-ADB-SHELL:~#
                        </span>
                      </div>
                    ...
                    <div className="flex-1 p-5 font-mono text-xs overflow-y-auto custom-scrollbar select-text flex flex-col gap-2 z-20 leading-relaxed">
                      <div className="text-slate-500 mb-2 border-b border-white/5 pb-2 select-none text-[11px]">
                        * Gõ các câu lệnh adb shell trực tiếp bên dưới (ví dụ:
                        pm list packages, dumpsys battery).
                        <br />* Hệ thống tích hợp cơ chế bảo vệ nâng cao chống
                        lại các câu lệnh phá hoại hệ thống.
                      </div>
                    ...
                    >
                      <span className="font-mono text-cyan-400 self-center font-black select-none text-sm">
                        $
                      </span>
                      <input
                        type="text"
                ...
                <div className="flex justify-between items-center pb-4 border-b border-slate-100 shrink-0 select-none">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5 font-semibold">
                      Theo dõi lịch sử câu lệnh ADB nâng cao đã được thực thi.
                    </p>
                  </div>
                  ...
                  {historyItems.length === 0 ? (
                    <div className="text-center py-24 text-slate-500">
                      ...
                      <p className="text-xs font-semibold">
                        Chưa có lịch sử câu lệnh nào được ghi nhận trong phiên
                        làm việc này.
                      </p>
                    </div>
                  ) : (
                      ...
                      {historyItems.map((item, idx) => (
                        ...
                        >
                          <div className="flex items-center justify-between mb-3 select-none">
                            ...
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded border font-mono ${getRiskBadgeStyles(
                                  item.risk,
                                )}`}
                              >
                                {item.risk}
                              ...
                              {item.success ? (
                                ...
                                <span className="flex items-center gap-1 text-[10px] font-black text-rose-600 bg-rose-500/10 px-2.5 py-0.5 rounded border border-rose-500/20">
                                  <XCircle className="w-3.5 h-3.5 text-rose-650" />{" "}
                                  THẤT BẠI
                                </span>
                              )}
                            </div>
                          ...
                          <div
                            onClick={() =>
                              handleCopyToClipboard(item.command, "câu lệnh")
                            }
