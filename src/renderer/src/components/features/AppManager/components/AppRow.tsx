...
export function AppRow({
  ...
  return (
    <div className="px-2 py-1">
      <div
        className={`flex items-center justify-between p-3 rounded-2xl transition-all h-[64px] ${isSelected ? "bg-blue-50 border border-blue-200/50 shadow-sm" : "hover:bg-slate-50 border border-transparent group"}`}
      >
        <div className="flex items-center gap-3 truncate w-1/2">
          <button
            onClick={() => onToggleSelect(pkg)}
            className={`w-5 h-5 rounded-md flex items-center justify-center transition-colors shrink-0 ${isSelected ? "bg-blue-600 text-white" : "bg-white border border-slate-300 text-transparent hover:border-blue-400"}`}
          >
            <CheckCircle2 className="w-3.5 h-3.5" />
          </button>
          <div className="truncate">
            {FRIENDLY_NAMES[pkg] && (
              <div className="text-[10px] font-black text-blue-600 uppercase tracking-tight mb-0.5">
                {FRIENDLY_NAMES[pkg]}
              </div>
            )}
            <div
