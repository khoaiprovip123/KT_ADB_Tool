...
export function useAppActions(
  ...
  const executeAction = async (
    ...
    try {
      return await withRetry(() =>
        window.api.manageApp(activeDevice, pkg, action),
      );
    } catch (err: any) {
  ...
  const handleSingleAction = async (
    ...
    if (action === "uninstall" && app.type === "system") {
      if (
        !window.confirm(
          `CẢNH BÁO: Gỡ ứng dụng hệ thống (${pkg}) có thể gây lỗi treo logo (bootloop). Bạn vẫn muốn tiếp tục?`,
        )
      )
        return;
    }
    ...
    try {
      ...
      if (res.success) {
        if (action === "uninstall")
          ...
          setPackages((prev) =>
            prev.map((p) => (p.pkg === pkg ? { ...p, status: "disabled" } : p)),
          );
        else if (action === "enable" || action === "restore")
          setPackages((prev) =>
            prev.map((p) => (p.pkg === pkg ? { ...p, status: "enabled" } : p)),
          );
  ...
  const handleExtract = async (pkg: string) => {
    ...
    try {
      ...
      const success = await withRetry(() =>
        window.api.extractApp(activeDevice, pkg, destPath),
      );
  ...
  const handleInstallApk = async () => {
    ...
    try {
      ...
      const success = await withRetry(() =>
        window.api.installApk(activeDevice, apkPath),
      );
      if (success) {
  ...
  const handleBatchAction = async (
    ...
    if (!explicitApps) {
      // Chỉ hỏi xác nhận nếu không phải là undo (explicitApps === undefined)
      if (action === "uninstall" || action === "disable") {
        const hasSystem = appsToProcessObj.some((a) => a.type === "system");
        if (hasSystem) {
          if (
            !window.confirm(
              `CẢNH BÁO: Bạn đang thao tác trên app hệ thống. Tiếp tục?`,
            )
          )
            return;
        } else {
          if (!window.confirm(`Thực hiện trên ${appsList.length} ứng dụng?`))
            return;
        }
      }
    }
    ...
    const removedPkgs: string[] = [],
      modifiedPkgs: { pkg: string; status: "enabled" | "disabled" }[] = [];
    ...
    for (let i = 0; i < appsToProcess.length; i++) {
      ...
      if (res.success) {
        ...
        if (action === "uninstall") removedPkgs.push(pkg);
          ...
          modifiedPkgs.push({ pkg, status: "disabled" });
        else if (action === "enable" || action === "restore")
          modifiedPkgs.push({ pkg, status: "enabled" });
      } else {
    ...
    if (successCount > 0) {
      ...
      if (action === "disable") {
        undoAction = {
          label: "Hoàn tác",
          onClick: () => handleBatchAction("enable", appsToProcess),
        };
      } else if (action === "enable") {
        undoAction = {
          label: "Hoàn tác",
          onClick: () => handleBatchAction("disable", appsToProcess),
        };
      }
