...
import { Package, Search, ListFilter, ChevronDown, Upload, RefreshCcw, User, Settings, Ban, Sparkles, CheckSquare, Square } from 'lucide-react'
...
export function AppManager() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full animate-in fade-in zoom-in-95 duration-500 relative overflow-hidden">
        ...
        <div className="flex flex-col gap-6">
          <div className="flex justify-center">
            <div className="inline-flex bg-slate-200/30 p-1 rounded-[2rem] border border-slate-200/50 shadow-inner">
              <FilterBtn active={filtering.filter === 'all'} onClick={() => filtering.setFilter('all')} label={`Tất cả (${loading ? '...' : packages.length})`} icon={<Package size={14} />} />
              <FilterBtn active={filtering.filter === 'user'} onClick={() => filtering.setFilter('user')} label={`Người dùng (${loading ? '...' : filtering.userCount})`} icon={<User size={14} />} />
              <FilterBtn active={filtering.filter === 'system'} onClick={() => filtering.setFilter('system')} label={`Hệ thống (${loading ? '...' : filtering.systemCount})`} icon={<Settings size={14} />} isWarning />
              <FilterBtn active={filtering.filter === 'disabled'} onClick={() => filtering.setFilter('disabled')} label={`Đã tắt (${loading ? '...' : filtering.disabledCount})`} icon={<Ban size={14} />} />
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            <StatCard label="Ứng dụng gốc" value={filtering.systemCount} icon={<Settings size={14} className="text-red-500" />} color="bg-red-50/50" />
            <StatCard label="Ứng dụng cài" value={filtering.userCount} icon={<User size={14} className="text-emerald-500" />} color="bg-emerald-50/50" />
          </div>
      ...
      <div className="flex-1 bg-white/60 backdrop-blur-3xl rounded-3xl border border-white/50 shadow-xl shadow-blue-900/5 flex flex-col relative z-0 overflow-hidden">
        <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-slate-100 bg-slate-50/50 shrink-0 items-center">
          <div className="col-span-6 flex items-center gap-3">
            <button onClick={selection.selectAll} className={`p-1 -ml-1 rounded-md transition-colors ${selection.allSelected ? 'text-blue-600' : 'text-slate-400 hover:text-blue-600'}`}>
              {selection.allSelected ? <CheckSquare className="w-5 h-5" /> : <Square className="w-5 h-5" />}
            </button>
      ...
      <BatchResultModal result={actions.batchResult} onClose={() => actions.setBatchResult(null)} />
    </div>
  )
}
