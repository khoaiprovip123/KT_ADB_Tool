...
interface ExperienceItemStatus {
  ...
  status: 'SUPPORTED_ON' | 'SUPPORTED_OFF' | 'UNSUPPORTED' | 'UNKNOWN' | 'ERROR'
  currentValue?: string
}
...
const FULL_XIAOMI_CAPABILITIES: XiaomiExperienceItem[] = [
  {
    ...
    description: 'Hiện số FPS/Tần số quét màn hình theo thời gian thực ở góc trái trên cùng.',
    category: 'display',
    risk: 'SAFE',
    defaultValue: '0'
  },
  {
    ...
    description: 'Ép màn hình chạy ở tần số quét cao nhất liên tục (ví dụ 120Hz), loại bỏ tự động tụt Hz.',
    category: 'display',
    risk: 'MEDIUM',
    defaultValue: '60'
  },
  {
    ...
    description: 'Bật chế độ bảo vệ mắt, giảm mỏi mắt khi sử dụng điện thoại vào ban đêm.',
    category: 'display',
    risk: 'SAFE',
    defaultValue: '0'
  },
  {
    ...
    description: 'Tăng tốc độ hoạt ảnh chuyển cảnh hệ thống lên gấp đôi (0.5x) giúp máy phản hồi cực kỳ nhanh nhạy và mượt mà.',
    category: 'display',
    risk: 'SAFE',
    defaultValue: '1.0',
    activeValues: ['0.5']
  },
  {
    ...
    description: 'Giảm độ trễ cảm ứng nhận vào, tăng cảm giác vuốt chạm tức thì mượt mà hơn.',
    category: 'navigation',
    risk: 'SAFE',
    defaultValue: '0'
  },
  {
    ...
    description: 'Ẩn thanh điều hướng cử chỉ phía dưới cùng để tối ưu không gian màn hình vô cực.',
    category: 'navigation',
    risk: 'SAFE',
    defaultValue: '0',
    isXiaomiOnly: true
  },
  {
    ...
    description: 'Tự động làm mờ nội dung preview các ứng dụng được chọn trong màn hình đa nhiệm để bảo mật thông tin.',
    category: 'launcher',
    risk: 'SAFE',
    defaultValue: '0',
    isXiaomiOnly: true
  },
  {
    ...
    description: 'Hiển thị số lượng RAM ảo mở rộng (Extension RAM) bên cạnh RAM thực tế ở góc đa nhiệm.',
    category: 'launcher',
    risk: 'SAFE',
    defaultValue: '0',
    isXiaomiOnly: true
  },
  {
    ...
    description: 'Tắt cơ chế tối ưu hóa độc quyền của MIUI. Giúp cài đặt file APK ngoài không bị lỗi, chạy đa nhiệm mượt mà hơn.',
    category: 'launcher',
    risk: 'RISKY',
    defaultValue: '1',
    activeValues: ['0'],
  ...
  },
  {
    ...
    description: 'Kích hoạt bộ rung phản hồi xúc giác tự nhiên và sắc nét trên toàn hệ thống.',
    category: 'sound',
    risk: 'SAFE',
    defaultValue: '1'
  },
  {
    ...
    description: 'Kích hoạt dải phím tắt tiện ích Game Turbo khi vuốt từ cạnh màn hình trong game.',
    category: 'game',
    risk: 'SAFE',
    defaultValue: '1',
    isXiaomiOnly: true
  },
  {
    ...
    description: 'Mở khóa giới hạn CPU/GPU của hệ thống để tối đa hóa FPS khi chơi game và xử lý tác vụ nặng.',
    category: 'game',
    risk: 'MEDIUM',
    defaultValue: 'normal',
    activeValues: ['high']
  },
  {
    ...
    description: 'Tắt thuật toán thu thập hành vi người dùng để phân phối quảng cáo mục tiêu trên toàn hệ thống Xiaomi.',
    category: 'ads',
    risk: 'SAFE',
    defaultValue: '1',
    activeValues: ['0'],
  ...
  },
  {
    ...
    description: 'Tắt các tin gợi ý, banner quảng cáo khó chịu trong các app Thư mục, Trình quản lý tập tin, Cài đặt.',
    category: 'ads',
    risk: 'SAFE',
    defaultValue: '1',
    activeValues: ['0'],
  ...
  },
  {
    ...
    description: 'Tắt dịch vụ phân tích dữ liệu chạy ngầm của Xiaomi nhằm cải thiện thời lượng pin và tăng tính riêng tư.',
    category: 'ads',
    risk: 'SAFE',
    defaultValue: '1',
    activeValues: ['0'],
...
export function UserExperience() {
  ...
  const isXiaomiDevice = deviceProfile && (
  ...
  const displayCapabilities = FULL_XIAOMI_CAPABILITIES.map(item => {
    ...
    const isUnsupported = realStatus?.status === 'UNSUPPORTED' || (deviceProfile && !isXiaomiDevice && item.isXiaomiOnly)
  ...
  const categories = [
    { id: 'all', label: 'Tất cả tweak', icon: <Sparkles className="w-4 h-4" /> },
    { id: 'display', label: 'Hiển thị & Màn hình', icon: <Eye className="w-4 h-4" /> },
    { id: 'navigation', label: 'Cử chỉ & Điều hướng', icon: <Sliders className="w-4 h-4" /> },
    { id: 'launcher', label: 'Launcher & Đa nhiệm', icon: <AppWindow className="w-4 h-4" /> },
    { id: 'sound', label: 'Âm thanh & Rung', icon: <Volume2 className="w-4 h-4" /> },
    { id: 'game', label: 'Game & Hiệu năng', icon: <Gamepad2 className="w-4 h-4" /> },
    { id: 'ads', label: 'Chặn QC & Gợi ý', icon: <Megaphone className="w-4 h-4" /> }
  ]
  ...
  const handleToggle = async (itemId: string, currentStatus: string) => {
    ...
    const executeToggle = async () => {
      ...
      try {
        const res = await window.api.applyXiaomiItem(activeDevice, itemId, nextEnable)
        if (res.success) {
          showToast(`Đã ${nextEnable ? 'kích hoạt' : 'vô hiệu hóa'} thành công: ${capItem.item.title}`, 'success')
  ...
  const handleRollback = async (itemId: string) => {
    ...
    const executeRollback = async () => {
      ...
      try {
        ...
        if (res.success) {
          showToast(`Đã khôi phục cài đặt gốc cho: ${capItem.item.title}`, 'success')
          await loadCapabilities()
        } else {
  ...
  const handleQuickOptimize = async () => {
    ...
    for (const c of safeOffItems) {
      try {
        const res = await window.api.applyXiaomiItem(activeDevice, c.item.id, true)
        if (res.success) {
          successCount++
        }
      } catch (error) {
    ...
    showToast(`Đã tối ưu hóa nhanh thành công ${successCount}/${safeOffItems.length} tùy chọn an toàn!`, 'success')
    await loadCapabilities()
    setLoading(false)
  }
  ...
  const supportedCapabilities = displayCapabilities.filter((c) => c.status !== 'UNSUPPORTED')
  const totalSupported = supportedCapabilities.length
  const enabledCount = supportedCapabilities.filter((c) => c.status === 'SUPPORTED_ON').length

  const safeCapabilities = displayCapabilities.filter((c) => c.item.risk === 'SAFE' && c.status !== 'UNSUPPORTED')
  const totalSafeCount = safeCapabilities.length
  const enabledSafeCount = safeCapabilities.filter((c) => c.status === 'SUPPORTED_ON').length

  const optimizationScore = totalSafeCount > 0 
    ...
    <div className="absolute inset-8 max-w-7xl mx-auto flex flex-col overflow-hidden bg-white/80 backdrop-blur-3xl rounded-[32px] p-5.5 border border-slate-200/60 shadow-[0_20px_50px_rgba(0,0,0,0.06)] text-slate-800">
      ...
      {!activeDevice ? (
        <div className="text-center p-12 bg-white/80 backdrop-blur-2xl rounded-[32px] border border-slate-200/50 shadow-xl flex-1 flex flex-col items-center justify-center min-h-[400px]">
          ...
          <h3 className="text-2xl font-black text-slate-800 tracking-tight mb-2">Chưa kết nối thiết bị</h3>
          <p className="text-slate-500 max-w-sm text-sm font-medium">Vui lòng cắm cáp kết nối thiết bị Android, bật Gỡ lỗi USB (USB Debugging) để bắt đầu cá nhân hóa giao diện.</p>
        </div>
        ...
        <div className="text-center p-12 bg-white/80 backdrop-blur-3xl rounded-[32px] border border-slate-200/50 shadow-xl flex-1 flex flex-col items-center justify-center min-h-[450px] relative overflow-hidden group">
          ...
          <div className="absolute w-72 h-72 rounded-full bg-amber-500/5 blur-3xl -bottom-20 -left-20 pointer-events-none animate-pulse" style={{ animationDelay: '2s' }}></div>
          ...
          {isUnauthorized && (
            <div className="w-full max-w-md bg-slate-50/80 border border-slate-200/50 rounded-2xl p-5 mb-8 text-left space-y-3.5 shadow-sm">
              <span className="text-[10px] font-extrabold text-rose-500 uppercase tracking-widest block mb-1">HƯỚNG DẪN XỬ LÝ NHANH</span>
              <div className="flex items-start gap-3">
                ...
                <p className="text-xs text-slate-650 font-semibold leading-relaxed">Mở khóa màn hình điện thoại Android của bạn.</p>
              </div>
              <div className="flex items-start gap-3">
              ...
              <div className="flex flex-wrap items-center gap-4 relative z-10 border-t lg:border-t-0 border-slate-150/60 pt-3 lg:pt-0">
                ...
                <div className="flex items-center gap-3 bg-white/90 backdrop-blur-sm px-4.5 py-2 rounded-2xl border border-slate-200/60 shadow-sm">
                  ...
                  <div className="relative w-9 h-9 flex items-center justify-center shrink-0">
                    ...
                    <svg className="w-9 h-9 transform -rotate-90">
                      <circle cx="18" cy="18" r="14" stroke="rgba(0,0,0,0.02)" strokeWidth="3" fill="transparent" />
                      <circle
                        cx="18"
                        cy="18"
                        r="14"
                        ...
                        strokeDashoffset={2 * Math.PI * 14 * (1 - optimizationScore / 100)}
                        strokeLinecap="round"
                        className="transition-all duration-1000"
                      />
                      <defs>
                        <linearGradient id="miniHUDGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                  ...
                  <div>
                    <span className="text-[8.5px] font-extrabold text-slate-400 uppercase tracking-widest block leading-none mb-0.5">HỆ THỐNG</span>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={loading || (totalSafeCount - enabledSafeCount) === 0}
            ...
            <div className="flex gap-1.5 overflow-x-auto custom-scrollbar pb-1 px-0.5">
              {categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    <span className={isActive ? 'text-cyan-300' : 'text-slate-450 group-hover:text-indigo-500'}>
                      {cat.icon}
                    </span>
                    <span>{cat.label}</span>
                  </button>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto pr-1 pb-4 custom-scrollbar flex flex-col">
            {loading ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-12 bg-slate-50/30 rounded-3xl border border-slate-200/50 min-h-[300px]">
                ...
                <p className="text-slate-400 text-xs font-bold">Không tìm thấy tùy chọn nào trong danh mục này.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4.5">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        {/* Title & Description */}
                        <h4 className={`font-black text-sm leading-snug group-hover:text-indigo-650 transition-colors ${isLocked ? 'text-slate-400' : 'text-slate-800'}`}>
                          {item.title}
                        </h4>
                      ...
                      <div className="flex items-center justify-between border-t border-slate-150/60 pt-3 mt-4 select-none shrink-0">
                        <div className="flex flex-col">
                          <span className="text-[8.5px] font-extrabold text-slate-400 uppercase tracking-widest leading-none mb-0.5">GIÁ TRỊ</span>
                          <span className="text-[10px] font-bold text-slate-600 font-mono truncate max-w-[100px] leading-tight">
                            {currentValue !== undefined && !isLocked && currentValue !== 'Chỉ hỗ trợ Xiaomi/Poco/Redmi' 
                              ? currentValue 
                              : '—'}
                          </span>
                        </div>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 max-w-sm pointer-events-none select-none">
        {toasts.map((toast) => (
          ...
          >
            ...
            <span className="text-xs font-bold text-slate-800 leading-tight">{toast.msg}</span>
          </div>
        ))}
      </div>
          ...
          <div className="bg-white border border-slate-200 shadow-2xl rounded-[32px] w-full max-w-md p-6.5 transform scale-100 transition-all">
            ...
            <div className="flex items-center gap-4 mb-5 pb-4 border-b border-slate-100 select-none">
              ...
              <div>
                ...
                <h3 className="font-black text-slate-800 text-lg mt-1 tracking-tight">Cảnh Báo An Toàn</h3>
              </div>
            </div>

            {/* Content text */}
            <div className="mb-6">
              <h4 className="font-extrabold text-slate-700 text-sm mb-1.5">{confirmModal.title}</h4>
