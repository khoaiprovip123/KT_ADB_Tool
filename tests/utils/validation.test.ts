...
describe("Validation Utility Tests", () => {
  describe("validateDpi", () => {
    ...
    it("should reject DPIs less than 160", () => {
      expect(validateDpi(159)).toEqual({
        valid: false,
        error: "DPI tối thiểu là 160",
      });
      expect(validateDpi(0)).toEqual({
        valid: false,
        error: "DPI tối thiểu là 160",
      });
      expect(validateDpi(-100)).toEqual({
        valid: false,
        error: "DPI tối thiểu là 160",
      });
    });
    ...
    it("should reject DPIs greater than 640", () => {
      expect(validateDpi(641)).toEqual({
        valid: false,
        error: "DPI tối đa là 640",
      });
      expect(validateDpi(1000)).toEqual({
        valid: false,
        error: "DPI tối đa là 640",
      });
    });
